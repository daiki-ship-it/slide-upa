 ...
 */
export function regenerateProject(projectDir, options = {}) {
  const { preserveOverrides = true } = options;
  ...
  const deck = {
    ...
    slides: parsed.slides.map(({ index, type, typeLabel, heading, script }) => ({
      index,
      type,
      typeLabel,
      heading,
  ...
  const audienceHtml = renderAudienceHtml(parsed.title, parsed.slides);

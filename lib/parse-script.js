...
function parseGoalItems(bulletLines) {
  ...
  for (const line of bulletLines) {
    ...
    if (numMatch) {
      ...
      num = numMatch2[1].charAt(0);
      text = line.slice(numMatch2[0].length).trim();
    } else {
    ...
    if (ch.sections.length > 0) {
      ...
      if (!skipChapterSlide) {
        ...
        slides.push({
          ...
          headingHtml: insertTitleBreaks(ch.heading),
          agendaItems: agendaItems.map((a, i) => ({ ...a, current: i === currentAgendaIndex })),
        });
      }

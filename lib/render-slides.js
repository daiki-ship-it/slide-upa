...
${footer}
    ...
    case "visual": {
      const visualBody = slide.visualBodyHtml ?? "";
      return `      <!-- ④ 画像メイン -->
...
${(slide.imageSlots ?? [])
  .map((slot, j) => {
    const imgSrc = slot.charFile
      ? `images/${slot.charFile}`
      : slot.generatedFile
        ? `images/${slot.generatedFile}`
        : null;
    if (imgSrc) {
      return `            <div
            ...
            >
              <img class="slide__visual-img" src="${imgSrc}" alt="">
            </div>`;
    }
    return `            <div

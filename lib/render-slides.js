...
${renderAgendaTimeline(s, slide.agendaItems ?? [], true)}
          </ol>
        </div>
${bot}
${footer}
...
${top}
        <div class="slide__body">
          ...
          <ol class="slide__timeline">
${renderAgendaTimeline(s, slide.agendaItems ?? [], false)}
          </ol>
        </div>
${bot}
${footer}
 ...
 * @param {string} s
 * @param {object[]} items
 * @param {boolean} highlightCurrent
 */
function renderAgendaTimeline(s, items, highlightCurrent) {
  return items
    .map((item, i) => {
      const current =
        highlightCurrent && item.current ? " slide__agenda-item--current" : "";
      return `            <li class="slide__agenda-item${current}" data-edit-id="${s}-a${i}">
              <span class="slide__timeline-dot" aria-hidden="true"></span>
              <span class="slide__agenda-label" data-edit-text>${escXml(item.label)}</span>
